    ...
    for inst in instances:
        try:
            ...
            if os.environ['EC2_PROVIDER'] == 'AWS':
                ...
                client.connect(inst.private_ip_address,
                               port=22,
                               username=host_by_ip[str(inst.private_ip_address)]['user'],
                               key_filename=host_by_ip[str(inst.private_ip_address)]['identityfile'][0])
            client.close()
            avail.append(True)
